...
import { jobRateLimiter } from "@/lib/rateLimiter";

/**
 * POST /api/jobs
 * Expects JSON body:
 * { name, email, phone, position, experience, resumeUrl }
 */
...
export async function POST(request) {
  try {
    ...
    const ip = request.headers.get('x-forwarded-for') || 
               ...
               'unknown';
    const rateLimitResult = jobRateLimiter.isAllowed(ip);
    ...
    // Validate form data
    const validation = validateJobForm(sanitizedData);
    ...
    // Insert into job_form table (singular)
      ...
      .insert([
        {
          ...
          resume_url: sanitizedData.resumeUrl,
        },
      ])
      .select();
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: "Failed to submit application. Please try again." }),
    ...
    try {
      ...
      await resend.emails.send({
        ...
        html: adminEmailHtml,
        // Note: Resend doesn't support attachments directly, but we include the resume link
      });
      ...
      console.error("⚠️ Failed to send job application emails:", emailError);
