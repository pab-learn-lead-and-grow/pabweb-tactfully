  ...
  useEffect(() => {
    const fetchData = async () => {
      const [mapRes, newsRes, catRes] = await Promise.all([
        supabase.from("news_category_map").select("*").eq("is_primary", true),
      ...
      const merged = mapData
        .map((m) => {
          ...
          if (imageUrl && !imageUrl.startsWith("http")) {
            const { data } = supabase.storage.from("News").getPublicUrl(imageUrl);
      ...
      const formattedTrending = merged
        .filter((item) => item.trending === true && item.is_published === true)
        .sort((a, b) => new Date(b.published_at) - new Date(a.published_at))
        .slice(0, 6);

      const formattedLatest = merged
        .filter((item) => item.is_published === true)
        .sort((a, b) => new Date(b.published_at) - new Date(a.published_at));

      setTrending(formattedTrending);
      setLatest(formattedLatest);
      setCategories(catData.sort((a, b) => a.category_name.localeCompare(b.category_name)));
    };
