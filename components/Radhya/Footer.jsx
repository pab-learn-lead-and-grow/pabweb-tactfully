...
import { getFooterPages, getNewsCategories } from "@/app/actions/getFooterData";
...
export default async function Footer() {
  ...
  for (let i = 0; i < pages.length; i += 8) {
    pageColumns.push(pages.slice(i, i + 8));
  }
  ...
  const programLinks = [
    ...
    {
      label: "Online B.Com(Int.)",
      path: "/programs/online-bcom-international/",
    },
  ...
  const universities = [
    ...
    { name: "Bennett Online", path: "/bennett/" },
    {
      ...
      path: "/online-universities-directory-india/",
    },
    {
      name: "Top Online Universities",
      path: "/top-online-universities-india/",
    },
  ];
  ...
  return (
    <footer className="w-full bg-white p-5 flex min-h-[90vh] xl:min-h-[50vh] items-center">
      <div className="mx-auto w-full max-w-7xl">
        <div className="grid grid-cols-1 lg:grid-cols-6 items-stretch gap-6 pb-4 border-b-2">
          {/* --- LEFT: People also search for --- */}
          <div className="lg:col-span-3 h-full">
            <h3 className="font-semibold text-xs md:text-sm text-black mb-2 flex items-center">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-2 w-full">
              {pageColumns.map((column, idx) => (
                <ul
                  key={idx}
                  className="space-y-1 text-[11px] md:text-xs text-black"
                >
                  {column.map((page) => (
                    <li key={page.page_id}>
                      <Link
          ...
          {/* --- RIGHT: stacked like your footer --- */}
          <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-3 gap-5 h-full">
            {/* News Categories */}
            <div>
        ...
        {/* --- Bottom Main Section --- */}
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-6 pb-2 border-b-2">
            ...
            <div className="col-span-2 flex justify-start items-start mt-4 text-left">
              <a href="/" className="block">
                <Image
                  ...
                  width={300}
                  height={60}
                  className="block w-45 lg:w-64 xl:w-70 h-auto"
                />
              </a>
            </div>

            {/* Social Icons */}
            <div className="flex justify-start space-x-2 mt-3 mt-8 mb-3">
              {[
                {
                  src: "/social/ig.webp",
                  alt: "Instagram",
          ...
          <div className="lg:col-span-4 grid grid-cols-2 md:grid-cols-4 gap-5 mt-5">
            {/* --- Quick Links --- */}
            <div>
              <h3 className="font-semibold text-xs md:text-sm text-black mb-2">
                Quick Links
        ...
        <div className="items-center justify-between text-[11px] md:text-xs text-black mt-3">
          <p>
            <b>Disclaimer: </b>Radhya Education Academy is an information and
            counselling platform owned and operated by Radhya Education Academy
            Private Limited. We are not a university or degree-granting
            institution. All course and university information provided on this
            website is sourced from official and publicly available platforms
            and may change without prior notice. Users are advised to verify
            details directly with the respective university before making any
            decision. We do not guarantee admission, placement, or outcomes and
            are not liable for any decision taken based on the information
