...
export default function ProgramsList({ title, categoryId }) {
 ...
 const CATEGORIES = [
  ...
  useEffect(() => {
    ...
    function updateItemsPerView() {
      ...
      const navButtonSpace = window.innerWidth >= 640 ? 96 : 0;
      ...
      setItemsPerView(Math.min(3, Math.max(1, fits)));
    }
  ...
  const nextCards = () =>
  setCardIndex((i) =>
    i + 1 > items.length - itemsPerView ? i : i + 1
  );
  ...
  return (
    <section className="p-5 lg:px-[10px] max-w-7xl relative">
      <h2 className="font-bold text-[28px] md:text-4xl lg:text-5xl xl:text-[64px] leading-tight text-[#270652] mb-6 text-center">
        {title}
      </h2>
